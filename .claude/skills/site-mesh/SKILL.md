...
| `post.improve_below`, `improve_rounds` | `0.3, 3` | 최악 셀 직접 수정: SICN이 목표 아래인 셀의 절점마다 후보 위치(이웃 중심 쪽 0.25/0.5/1, 맞은편 면 법선 쪽 0.1/0.25/0.5·L)를 시험해 그 절점 둘레 셀들의 최소 SICN이 가장 오르는 것을 채택. 내부 절점은 자유, 한 평면·한 패치 위 경계 절점은 평면 안에서만(형상·패치 면적 보존), 패치 경계·곡면·모서리 절점은 고정. 로그 `worst cells (SICN < …)` |
...
| `trim.shrink_xy_m` | `0` (전체 ±1,250 m) — 자체 솔버 수렴 시험 때만 `600`(±650 m) | 원 STEP ±1,250 m의 먼 곳은 지형 계단·기저 경사·벽-출구 접합 쐐기가 무한히 나와 스테디 해가 매번 다른 곳에서 터졌다; ±650 m에서 50회 무발산. **납품 형상엔 트림을 걸지 않는다**: ±650 m는 북서쪽 산(지형 z 85 m)을 통째로 잘라 사용자가 거부했다(2026-09-10). 전체 도메인은 `max 20`(성장 필드가 700 m에서 상한 도달)으로 외곽 링을 20 m급으로 두면 셀 수가 ±650 판과 같다(1,610만); 대신 먼 지형 쐐기 27곳 때문에 min SICN 0.045·0.2 미만 203개(전부 650 m 밖)가 남는다 |
