...
**정정.** 이 문단은 오랫동안 "`ofgpu-buoyant` 등 부력 계열 드라이버"라고
적혀 있었지만 `ofgpu-buoyant`는 JSONC를 읽지 않습니다 —
...
`docs/07-lowmach-solver.md` §1.1에 기록된 모든 측정이 균일 형태로 이루어졌고
...
| Case | 드라이버 | 설명 |
|---|---|---|
| `dieStack.cht.jsonc` | `ofgpu-cht` | **SPEC-LIT §46/§47** — 켤레 열전달(conjugate heat transfer)의 첫 케이스이자, 이 저장소에서 처음으로 **여러 영역(region)** 을 하나의 열 메쉬로 이어 붙여 푸는 케이스입니다. 100 W를 소산하는 10 x 10 x 0.7 mm 실리콘 다이 → 솔더 TIM1 → 구리 스프레더 → 그리스 TIM2 → 등온 냉각판, 접촉저항 3개(§47.5)와 메쉬축 대각 이방성 전도도 (§46.3, 면내 120 / 두께방향 30 W/(m K))를 포함합니다. **확장자 `.cht.jsonc`** 는 이 문서의 다른 모든 케이스가 쓰는 유동 케이스 형식(`io::case_json`)이 아니라 다영역 전도 형식(`io::case_cht`)임을 뜻하며, 두 형식의 shipped-case 검사도 서로 나뉘어 있습니다. 스택 전체가 1차원이므로 **닫힌 해**가 있고 실측이 그것을 재현합니다: 접합부 온도 **649.7118 K**(닫힌 해 `300 + q(3.380452e-4) + 11.6667`, 상대오차 1e-8 이내), 세 계면 각각 **−100.000000 / +100.000000 W**, §47.12 Gate 4 보존 불균형 **0.000e0**, 최대 계면 온도 **점프** 40.000000 K = `q * 4e-5`(§47.3의 Robin 삼중항이 불연속을 실을 수 있기 때문 — cyclic 보간으로는 불가능). **§13.4.1 짝 시험**: 접촉저항 하나를 `2.5e-5` → `7.5e-5`로 바꾸면 접합부가 정확히 `q dRc` = 50 K 올라갑니다. 실행: `cargo run --release --bin ofgpu-cht -- ..\cases\dieStack.cht.jsonc -csv dieStack.csv` **열응력(SPEC-LIT §96)**: 다이·솔더·스프레더가 `mechanics` 블록을 얻었고 `run.mode: "stress"`로 돌면 전도 해 뒤 각 영역의 선형 열변형이 풀리고 영역별 VTU가 `<case>_jsonc/VTK/`에 쓰입니다(그리스는 T만). 실측: 다이 max|u| 1.2112e-5 m / 최대 폰 미제스 2.68e6 Pa, 솔더 1.4789e-4 m / 2.36e8 Pa, 스프레더 9.1757e-5 m / 6.18e8 Pa — 케이스 헤더가 밝히는 세 이상화 아래의 **선형탄성 비교 수치**이며 열인장이므로 닫힌해 시험은 그대로 통과 |
| `bimetalStrip.cht.jsonc` | `ofgpu-cht` | **SPEC-LIT §95/§96** — 바이메탈 온도계 띠의 티모셴코(1925, DOI 10.1364/JOSA.11.000233) 폐곡률 케이스. 60 x 10 x 0.625 mm의 강/황동이 **한 영역**의 `materials` 존 목록(기본 `series` 결합, §95.8)으로, z 한 셀이라 앞·뒷면이 실제 traction-free → 평면응력 그대로. 양끝 303.15 K, 추적 열원으로 실제 정상 해가 10 K 승온. 실측(RTX 5070 Ti, f64): 마지막 x열 평균 팁 처짐 **−2.1291e-5 m** 대 닫힌 해 **−2.0728e-5 m**(**2.7 %**, 시험이 단언하는 10 % 밴드 안, 부호 −y = 황동이 볼록면), 565 외부 반복. **§13.4.1 짝 시험** 열 개가 같은 경로를 검증(`io::case_cht::tests::pair_*`): E, ν, TRef, 트랙션, 고정변위, 대칭, tolerance, maxOuter, 결합 처리 — 각각 하나를 바꾸면 답이 바뀌거나 거부됨. 실행: `cargo run --release --bin ofgpu-cht -- ..\cases\bimetalStrip.cht.jsonc` |
| `kaminskiPrakash.cht.jsonc` | `ofgpu-cht` | **SPEC-LIT §59/§60** — 이 저장소의 첫 **켤레 유체/고체(conjugate fluid-solid)** 케이스이자, §47.12가 이름 붙이고 §47.14가 “실행되지 않음”으로 기록해 두었던 **Gate 5**(Kaminski & Prakash 1986, DOI 10.1016/0017-9310(86)90017-7)를 실제로 돌리는 케이스입니다. 1 x 1 정사각 밀폐공간 — 왼쪽 `0 <= x <= 0.2`는 전도하는 고체 벽(바깥면 `T_h` 고정), 나머지 `0.2 <= x <= 1`은 공기(먼 쪽 면 `T_c` 고정), 위/아래는 고체·유체 모두 단열. `regions[1].material.kappa`가 곧 **전도도비 `Kr`**입니다(유체의 `kappa`가 1이므로). 무차원화: `rho = cp = kappa_f = 1`로 `alpha = 1`, `mu = 0.71`로 `Pr = 0.71`, 따라서 `buoyancy.g[1] = -2130 Ra`이고 파일에 적힌 `-2.13e8`은 **`Ra = 1e5`**입니다. `dT/TRef = 3.3e-4`이므로 §9의 `g(TRef/T - 1)`가 Boussinesq에서 벗어나는 몫은 `3.3e-4`입니다(가정이 아니라 계산된 값, §59.9).
　　**유체 영역은 닫힌 공동(closed cavity)입니다**: `empty`가 아닌 모든 패치는 no-slip 벽이고, 입구/출구는 이름으로 거부됩니다(§60.2). 그것이 §47.12의 Gate 6(Qu & Mudawar 2002, 마이크로채널 히트싱크)이 아직 이 형식으로 표현조차 되지 않는 이유이며 §60.6이 그렇게 적어 둡니다.
　　**실측(RTX 5070 Ti, f64).** 평균 Nusselt 수는 (S60.1)의 `Nu = |Q| L/(k_f dT H d_z)`이고, 드라이버가 찍는 **냉각벽 / 가열벽 / 계면** 세 열유량에서 각각 독립적으로 계산됩니다 — 정상상태에서 셋은 같은 수이며, 그 퍼짐이 이 케이스 자신의 수렴 척도입니다. 이 파일이 적은 `Ra = 1e5`, 80x80에서 `Kr = 0.1 / 1 / 10`(= `regions[1].material.kappa`) → `Nu = 0.42507 / 2.24840 / 4.21565`이고, 같은 격자에서 `Ra = 1e4`이면 `0.38080 / 1.52290 / 2.26916`입니다. 60x60 → 80x80 격자 변화는 모든 점에서 **0.38 % 이하**. Belazizia 등(2012)의 표(§60.5가 **2차 출처**임을 명시)에 대해 `Ra = 1e5`에서 **-7.79 % / -4.32 % / -0.81 %**, `Ra = 1e4`에서 **-7.12 % / -3.00 % / -0.48 %** — 전도가 지배하는 `Kr = 0.1`에서 벌어지고 대류가 지배하는 `Kr = 10`에서는 사실상 일치합니다. §60.5가 그 진단과, **Kaminski & Prakash 원논문 자체는 유료 장벽 뒤에 있어 표를 읽지 못했다는 사실**을 기록합니다. 함께 도는 게이트: **Gate 59-B**(전도 극한 `Nu = 1/(D/Kr + 1 - D)`를 `1e-8` 이내로 재현 — 출판 데이터가 전혀 필요 없는 해석해)와 **Gate 59-A**(de Vahl Davis 1983 정사각 공동, 같은 형식·같은 솔버에서 `Ra = 1e3/1e4/1e5` → `1.1186 / 2.2505 / 4.5458`, 출판값 `1.118 / 2.243 / 4.519` 대비 `+0.06 / +0.34 / +0.59 %`). 실행: `cargo run --release --bin ofgpu-cht -- ..\cases\kaminskiPrakash.cht.jsonc -csv kp.csv` |
| `plume.jsonc` | `ofgpu-k-epsilon -permissive` | `plumeB`(OpenFOAM 형식)의 JSONC 재현 — 두 형식이 같은 필드를 만든다는 B3 게이트. **`-permissive`가 필요해졌습니다(SPEC-LIT §13.4)**: 이 케이스는 `physics.gravity [0, 0, -9.81]`을 이름하는데(짝인 `plumeB`의 `constant/g`도 같습니다), `ofgpu-k-epsilon`은 얼린 `U` 위에서 난류 두 방정식만 풀고 온도장을 읽지 않으므로 §17의 부력 생성 `G_b = (nu_t/Pr_t) g·grad(T)/T`를 만들 재료가 없습니다. `KEpsilon::set_buoyancy`는 처음부터 있었고 아무도 부르지 않았습니다 — 즉 이 명령은 **예전에도 `G_b`를 0으로 두고 돌았고, 그 사실을 말하지 않았을 뿐입니다**. `-permissive`는 무엇을 대체했는지 출력하고 예전과 **비트 단위로 같은** 결과를 냅니다(측정: `cases/channel`·`cases/channelKW`·`plumeB` 모두 수정 전후 필드 파일이 완전히 동일). 부력이 실제로 필요하면 `ofgpu-lowmach`(JSONC)나 `ofgpu-plume`/`ofgpu-buoyant`(디렉터리)를 쓰십시오 |
| `channelThermalWF.jsonc` | `ofgpu-lowmach` | SPEC-LIT §29.3/§30.3의 게이트, 메쉬 (a): 2.0 m(L/Dh = 50) 입구구동 평면 채널/덕트, 위아래 벽 `Tw = 373.15 K`, 중력 0. 벽법선 방향 균일 격자(6칸) — 측정 y+ 21.95/37.90/40.29(목표 30-60), `standard` 프리셋, `thermalWallFunction`이 실제로 일을 해야 하는 조건. **재실행(SPEC-LIT §13.4.1)**: 고쳐진 드라이버로 6,000 반복을 다시 돌려 y+ **21.95/37.76/40.31**, 총 벽 열유입 **157.035 W**, `T_b` 308.039 K, `U_b` 3.00469 m/s, `\|U\|` 잔차 9.4e-10. 위 y+는 사실상 변하지 않았다 **재실행(드라이버 분리, SPEC-LIT §26.1 이후 처음)**: 이 행의 숫자는 §25.1의 발산 구속조건에 전도항이 들어가기 전(§26.1 이전)에 기록된 것이고, 두 flux 쌍둥이와 달리 그 뒤로 다시 돌린 적이 없다. 같은 명령을 오늘 다시 돌린 결과: y+ **21.943/39.119/43.298**, 총 벽 열유입 **161.539 W**(157.035 W에서 +2.9%), `T_b` **308.064 K**, `U_b` **3.15454 m/s**(3.00469에서 +5.0%), `|U|` 잔차 9.6e-10. **이 이동은 드라이버 분리 때문이 아니다** — 분리 전후의 두 바이너리가 이 케이스에서 출력 한 줄까지 동일하다(측정). 위 숫자들은 기록으로 남기되 현재 값이 아님을 여기에 표시한다(SPEC-LIT §13.4.3) |
...
| `channelPeriodicWF.jsonc` | `ofgpu-lowmach` | SPEC-LIT §31의 페리오딕 재시도, 메쉬 (a): 위 두 케이스와 같은 단면(0.04 m × 0.04 m)·`Tw`를 스트림방향 **cyclic**(`mesh.cyclic`, 0.08 m, 8칸)으로 바꾸고, 유입 대신 `sources[]` `momentumSource`(체적력 3.9 m/s²)로, 에너지는 `-heaterPower -6`(균일 도메인 열싱크)로 구동 — 벽법선 균일 6칸, `standard`/`thermalWallFunction`. 측정 y+ 40.25/41.73/43.41, 완전 수렴(`\|U\|` 잔차 1.3e-10). **재실행(SPEC-LIT §13.4.1)**: 고쳐진 드라이버로 3,000 반복을 다시 돌려 y+ **40.14/41.80/43.64**, 총 벽 열유입 **6.00555 W**(싱크 −6 W와 §31의 에너지 균형이 여전히 닫힘), `T_b` 314.243 K, `U_b` 3.53146 m/s, `\|U\|` 잔차 1.0e-9 — y+도 열균형도 실질적으로 변하지 않았다 **재실행(드라이버 분리, SPEC-LIT §26.1 이후 처음)**: 이 행도 §26.1 이전 기록이다. 오늘 같은 명령으로 다시 돌린 결과: y+ **40.138/41.791/43.636**, 총 벽 열유입 **6.00374 W**(6.00555 W에서 −0.03%), `T_b` **314.255 K**, `U_b` **3.53192 m/s**, `|U|` 잔차 4.9e-8(1.0e-9에서). 열균형은 여전히 닫힌다(싱크 −6 W 대 벽열 6.00374 W). **드라이버 분리 때문이 아니다** — 분리 전후의 두 바이너리가 출력 한 줄까지 동일하다 |
...
cargo run --release --bin ofgpu-lowmach -- ..\cases\channelPeriodicFluxLowRe.jsonc -iters 40000 -check 5000
...
#
# 판정(자세한 것은 docs/07-lowmach-solver.md §1.1의 마지막 소절):
...
# ofgpu-lowmach는 이제 벽면 전단력에서 f를 직접 측정하고, 힘 균형을 이 크레이트가
# 실제로 조립하는 운동량 방정식과 같은 운동학적 단위(m4/s2)로 비교해
# 출력합니다(SPEC-LIT §32.5.2의 보정).
# docs/07-lowmach-solver.md §1.1에 전체 표와 진단이 있습니다.
```

저-마하 솔버 설명과 네 채널 케이스가 실제로 낸 벽 열유속·비율은
[`../docs/07-lowmach-solver.md`](../docs/07-lowmach-solver.md)를 보십시오.
