...
`ofgpu-lowmach`, `ofgpu-fire`와 `ofgpu-k-epsilon`은 OpenFOAM 케이스 디렉터리 대신 주석과
...
| `nistBurner37cm.jsonc` | `ofgpu-fire` | **SPEC-LIT §62.12 Gate 4** — §61(그을음)과 §62(WSGG 분광 복사)의 화재 게이트. NIST 37 cm 수냉식 프로판 버너(Sung, Chen, Bundy, Fernandez, Hamins, **NIST TN 2162r1**, 2021), 20/34/50 kW에서 **측정 복사분율 0.23 / 0.30 / 0.33**(FDS Validation Guide `Experiment_Chapter.tex`, NIST 공개영역). 게이트는 **레벨이 아니라 추세**를 요구합니다 — 상수 `a`를 조정한 회색 모델은 한 점은 맞출 수 있지만, 하나의 설정으로 세 점을 모두 맞출 수 있는 것은 조성 의존 모델뿐입니다. 원형 버너는 등면적 정사각형(10×10셀, 0.3333 m, 등가지름 0.3761 m 대 실측 0.37 m)으로 대체했고 그 근거는 파일 헤더에 있습니다. 20 kW / 50 kW leg은 이 파일에서 `inlet.U`만 바꾼 것(헤더의 표)입니다. 이 케이스는 **fvDOM + WSGG + prescribedYield**를 `updateInterval: 4`로 돌리며, 둘 다 선택이 아니라 강제된 것입니다 — P1 + WSGG는 바로 이 메쉬에서 NaN으로 발산했고(§62.5의 투명창 특이성이 주장이 아니라 측정이 된 순간), 120번의 수송 풀이/호출을 165,888셀에서 매 스텝 돌리면 HRR당 몇 시간입니다. 실행: `-endTime 6.0 -deltaT 0.005`. **이 게이트는 실패합니다**: 연소효율이 226 %로 나와 연료 공급량보다 많은 열을 내므로(=누적된 연료 재고를 태우는 중) 복사분율을 측정값과 비교할 수 있는 상태가 아니며, 그을음을 켜면 `Y_s`가 발산합니다(§61.2가 이미 이름 붙인 한계). 모든 실행 기록과 진단은 파일 헤더와 SPEC-LIT §62.13, `docs/07-fire-solver.md`에 있습니다 |
