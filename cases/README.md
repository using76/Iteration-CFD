...
| `burnerPlume_fvDOM.jsonc` | `ofgpu-fire -combustion -radiation` | `burnerPlume.jsonc`와 메쉬·유동·연소가 완전히 동일한 SPEC-LIT §36 fvDOM 쌍둥이 케이스 — `physics.fire.radiation.model`만 `"fvDOM"`으로 다름. 같은 1200스텝(`-endTime 6.0 -deltaT 0.005`)에서 복사 분율 P1 **14.98%** 대 fvDOM **13.83%**(둘 다 도메인 열방출 대비) — 벽시계 시간은 P1 **19.08 s** 대 fvDOM **124.6 s**(RTX 5070 Ti, 32768 셀, 배수 6.5) — SPEC-LIT §36.6가 명시한 "N_ordinates배 비용"이 실측치로 확인됨. **재실행(SPEC-LIT §13.4.1)**: 예전 기록은 P1 15.08% / fvDOM 13.35%, 18.8 s / 119 s였다. 그 숫자들은 이 케이스의 `numerics` 블록을 전혀 읽지 않는 드라이버가 낸 것이다(운동량이 `bounded Gauss upwind`로 돌았고 케이스는 `Gauss linearUpwind grad(U)`를 요구한다). 두 모델 모두 고쳐진 드라이버로 다시 돌렸고, 이 행이 주장하는 **P1 대 fvDOM 비교 자체는 그대로다**(fvDOM이 여전히 더 적게 복사하고, 차이는 1.73포인트에서 1.15포인트로). 비교의 대상이 되는 상태 자체는 크게 움직였다 — `docs/07-fire-solver.md` §6 참고 |
...
| `channelThermalLowRe.jsonc` | `ofgpu-fire` | 위와 동일한 형상·유입 조건·벽온도, `mesh.grading`으로 벽법선 방향을 양쪽 벽 모두를 향해 two-sided grading(`expansion: 200`, 50칸, 첫 셀 높이 ≈ 2×10⁻⁵ m)하여 y+ 목표 ≈ 1(측정 0.43/0.89/1.02)을 노리는 `lowRe` 프리셋 — 벽은 순수 분자 저항의 평범한 `fixedValue`(SPEC-LIT §29.3: "lowRe는 해상된 서브레이어 자체의 분자 저항을 그대로 둔다"). 두 케이스의 벽 열유입 비율은 0.381(첫 시도의 0.095에서 4배 개선, 여전히 게이트는 열려 있음) — 자세한 내용은 `docs/07-fire-solver.md` §1.1. **⚠ 지금은 그대로 실행되지 않으며, 재실행하지 않았다.** 이 파일은 `wallTreatment lowRe`와 `turbulence.model kEpsilon`을 함께 이름하는데, 나중에 들어온 SPEC-LIT §33 규칙이 그 조합을 §13.4 오류로 거부한다(`LaunderSharmaKE`를 안내). 위 0.381·y+ 0.43/0.89/1.02는 **§33 규칙 이전에, 그리고 §13.4.1 numerics 수정 이전에** 측정된 값이며, 이 시도 자체가 `channelPeriodicFluxWF/LowRe`로 대체된 폐기된 게이트라서 케이스 파일을 고쳐 되살리지 않았다 — 기록으로만 남긴다 |
...
# ⚠ 아래 두 명령은 **지금은 실행되지 않습니다**. 두 케이스가 `wallTreatment lowRe`와
#   `turbulence.model kEpsilon`을 함께 이름하는데, 나중에 들어온 SPEC-LIT §33 규칙이
#   그 조합을 §13.4 오류로 거부합니다("available: LaunderSharmaKE"). 두 케이스가 낸
#   숫자(비율 0.381 등)는 §33 규칙 이전, 그리고 §13.4.1 numerics 수정 이전의 기록이며,
#   이 시도 자체가 아래 channelPeriodicFlux* 쌍으로 대체된 폐기된 게이트라서 케이스를
#   고쳐 되살리지 않고 기록으로만 남깁니다. `-permissive`도 이 기록을 재현하지
#   못합니다 — 그 플래그는 `standard`(벽함수)로 대체하지 `lowRe`로 돌리지 않습니다.
