...
- show_chart {chart, runId}: open a chart panel (residuals | metrics | surface); open_result {path, timeIndex, region}: load a result (or one region of a multi-region case) into the viewer.
...
- post_warp {field, scale}: deform the drawn surface by a point displacement field (scale 1 = true deformed shape; field null or scale 0 removes it).
...
- geometry_open {path} / geometry_import_step {path}: open a surface (or a STEP, one part per solid) in the Geometry tab — these gui_control forms put it ON SCREEN; the server tools of the same names only read it for you; geometry_part {name, action, newName}: show, hide, keep_only, drop, select or rename a part; geometry_transform {op, value, pivot}: translate (metres), rotate (degrees), scale, mirror, undo, redo, reset — the view shows it live, nothing is written; geometry_boolean {op, a, b}: fuse, cut or common of two parts of a STEP; geometry_save {path, binary, keepVisibleOnly, overwrite}: write the edited surface. gui_state's geometry section says what is open, its parts and whether it is closed.
