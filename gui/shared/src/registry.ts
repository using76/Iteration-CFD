...
export const BINARIES: BinarySpec[] = [
  ...
  {
    name: 'ofgpu-convert-mesh',
    source: 'src/bin/convert_mesh.rs',
    purpose: 'Read a Gmsh MSH 4.1 ASCII mesh and write it as an OpenFOAM-format polyMesh (physical surfaces become patches, typed by the wall_/empty_/symmetry_ name prefixes or -type) and, on request, as an ANSYS Fluent mesh.',
    summary: 'Convert a Gmsh .msh to a solver case, and to a Fluent mesh.',
    kind: 'mesh',
    positionals: [
      { name: 'msh', type: 'path', description: 'Gmsh MSH 4.1 ASCII file with physical groups.' },
      { name: 'outCaseDir', type: 'path', description: 'Case directory that receives constant/polyMesh (an existing polyMesh is refused).' },
    ],
    flags: [
...
export const PIPELINES: BinarySpec[] = [
  ...
  {
    name: 'geom-tool',
    source: 'tools/geom/geom_tool.py',
    purpose: 'Run the geometry tool (tools/geom/geom_tool.py, gmsh) on a STEP/BREP/XAO/IGES/STL file: info lists the solids as JSON (tags, names, materials, CAD volumes), export writes the file back by extension (.step/.brep/.xao as solids, .stl as a binary surface mesh), edit applies an ops JSON and writes --out.',
    summary: 'Geometry tool: info, export and edit on a CAD file (solid list JSON, STL surface mesh, ops JSON).',
    kind: 'mesh',
    positionals: [
      { name: 'command', type: 'enum', values: ['info', 'export', 'edit'], description: 'info lists the solids of a STEP/BREP/XAO file (surfaces for IGES, discrete surfaces for STL); export writes it by extension (mm STEP, BREP, XAO, or a binary STL surface mesh); edit applies an ops JSON and writes --out' },
      { name: 'file', type: 'path', description: 'The geometry file (workspace path)' },
    ],
    flags: [
      { name: '--json', type: 'path', description: 'info: write the solid list as JSON here (file only, never stdout)' },
      { name: '--out', type: 'path', description: 'export/edit: the output file; the extension picks the format' },
      { name: '--ops', type: 'path', description: 'edit: the ops JSON (version 1, fourteen op names)' },
      { name: '--scale', type: 'float', description: 'File units to metres (default by extension: STEP/IGES 0.001, BREP/XAO/STL 1.0); must be > 0' },
      { name: '--stl-size', type: 'float', description: 'export to .stl: surface-mesh size in metres (default: bounding-box diagonal / 20)' },
    ],
    accepts: [],
    builds: [],
    residualStyle: 'none',
