...
```
cargo test        1509 passed, 0 failed, 4 ignored (lib 크레이트 기준)
                  1657 passed, 0 failed, 6 ignored (모든 타깃 합계 — 각 바이너리의 CLI 파싱
                  스위트와 SPEC-LIT §13.4.1의 드라이버별 "두 실행은 달라야 한다" 쌍 검사 포함)
ofgpu-validate    813 / 813 checks passed (765개는 실시간 계산, 48개는 기록된 측정값 재생).
