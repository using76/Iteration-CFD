...
| 자선단체 | **무료** |
| 공공 목적 연구 — 소방·의료·보건·안전·재난·환경 | **무료** (정부출연연구기관 포함) |
| **연구기관의 기술개발·기반연구 (정출연 등)** | **30일 시험** 후 유상 라이선스 |
| 그 밖의 상업적 이용 | **30일 시험** 후 유상 라이선스 |

**「공공」은 기관이 아니라 연구의 목적을 가리킵니다.** 소방·의료·보건·안전·재난·환경
보호를 목적으로 하는 연구는 정부출연연구기관에서 수행하더라도 무료입니다. 반면 특정
제품이나 산업 이전을 목적으로 하는 기술개발 — 전기차, 철도·추진, 항공기 엔진 기술 등 —
은 정출연에서 수행하더라도 상업적 이용입니다. 실무 기준은 하나입니다: **기술료 징수
대상이거나, 기업을 위해 또는 결과를 기업에 이전하는 사업으로 수행되는 연구는 유상**
입니다. 대학·학교 및 그 소속 연구소는 교육기관으로서 무료입니다.
...
설치부터 첫 케이스, 격자, 솔버 선택, 수렴 판독, 결과 보기, 그리고 자주 만나는
오류의 원인까지 — **쓰는 법**을 다루는 문서는 [`docs/GUIDEBOOK.md`](docs/GUIDEBOOK.md)
하나입니다. 영어판은 [`docs/GUIDEBOOK.en.md`](docs/GUIDEBOOK.en.md).

가장 많이 틀리는 곳부터 보시려면 6절 「솔버 고르기」입니다 — `ofgpu-k-epsilon`,
`ofgpu-k-omega`, `ofgpu-sa`는 **속도장을 얼린 채 난류 방정식만** 풉니다.
...
**그리고 빗나가는 게이트 두 개.** `ofgpu-validate`가 매 실행 이름으로 부르며, 아래는 그 이름 그대로입니다.
...
각 검증 절이 어떤 종류의 검증이고 **무엇에 견준 것인지**(논문·벤치마크 수치·상관식·공개 데이터·저장소의 기록), 그리고 그 자료의 접속 주소(DOI/URL)는 [`docs/GUIDEBOOK.md`](docs/GUIDEBOOK.md) §11 「검증 항목 하나하나」의 표에 있습니다.
