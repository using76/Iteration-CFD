...
| 복사 | 회색 P1 근사 (Modest ch. 15), Marshak 벽 경계, `chi_r` 복사분율 하한, **그리고 회색 fvDOM** (Modest ch. 16; Fiveland 1984; Truelove 1987 — SPEC-LIT §36): 같은 RTE를 24개 level-symmetric S4 종좌표로 풀며 `radiationModel`로 선택. `cases/burnerPlume.jsonc` 실측(32,768셀, 1,200스텝, RTX 5070 Ti): 복사 분율 14.98%(P1) 대 13.83%(fvDOM), 벽시계 19.08 s 대 124.6 s |
...
| 해상 leg 메쉬 해상도(재현) — `cases/channelPeriodicFluxLowRe.jsonc`의 최악 벽인접 y+와 y+<20 셀 수 | y+ = 0.00185, 400개 중 192개 (두 조건 모두 충족) |
| 해상 leg Nusselt 검증치(재현) — 같은 케이스, 같은 두 상관식 | +14.1% / +6.0% (DB 밴드 안쪽, Gnielinski 밴드 밖이고 이 leg의 ±3.1% 에너지 불균형으로도 밴드에 닿지 않음 — **닫히지 않음**) |
| Thermostat 가중, 결정적 실험(재현) — `"weighting"` 토큰 하나만 바꾼 네 번의 실행 | `massFlux`가 두 leg 모두에서 `Nu`를 낮추고 `T_w − T_b`를 넓히며, 해상 메쉬를 벽함수 메쉬보다 2.7배 더 움직임(−3.54% 대 −1.32%) |
| 운동량의 bounded 대류, 고립 실험(재현) — `div(phi,U)` ∈ {`Gauss upwind`, `Gauss linearUpwind grad(U)`} × {평문, `bounded`} 일곱 번의 실행 | `bounded`를 떼면 두 leg 모두 운동학적 힘 균형이 닫힘(−3.787% → −0.000%, −0.112% → −0.005%); 스킴의 **차수**는 `Nu`의 0.3% 미만 |
...
|---|---|---|
| y+ (벽인접) | 56.89 / 57.78 / 58.59 | 0.00185 (400셀 중 192셀이 y+ 20 미만) |
| `T_w`(진단) / `T_b`(혼합평균) | 317.483 K / 293.251 K | 314.186 K / 292.800 K |
| `U_b` / Re | 5.3972 m/s / 28,785 | 4.92909 m/s / 26,288 |
| **Nu (측정)** | **64.5257** | **72.9988** |
| Petukhov 원관 `f`의 Gnielinski (**절대 예측**) | −5.9% — **±10% 안쪽** | +14.1% — **밖** |
| Dittus-Boelter | −12.9% — ±20–25% 안쪽 | +6.0% — 안쪽 |
| 이 leg가 **측정한** `f`의 Gnielinski (**레이놀즈 유사**) | +34.4% — 밖 | +16.6% — 밖 |
| 에너지 균형 (thermostat 출력 대 측정 벽열) | +0.106% | +3.11% |
...
**판정, 한 번만 정리하면.** §32.4의 절대 예측 판정에서 벽함수 leg는
**닫히고** 해상 leg는 **닫히지 않습니다** — 그리고 해상 leg의 미스는 이제
경계선이 아니라 확정적입니다. 이 leg 자신의 ±3.1% 에너지 불균형
불확실성(Nu ∈ [70.7, 75.3])으로도 Gnielinski 밴드 가장자리에 닿지 않기
때문입니다. 각 leg의 벽이 **측정한** `f`로 평가하는 레이놀즈 유사 판정에서는
...
오차라는 점도 맞습니다. **다만 이것은 가설이며, 아직 아무것도 측정하지
않았습니다.** 같은 leg의 +3.11% 에너지 불균형이 남은 다른 하나인데,
`Energy::assemble`이 bounded 보정을 **무조건** 적용하므로(§26) 케이스 설정
으로는 분리할 수 없고, 다음 실험은 코드 변경을 필요로 합니다.
