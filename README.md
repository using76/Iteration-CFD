...
| 열 벽함수 게이트, Nusselt 검증치(재현) — `cases/channelPeriodicFluxWF.jsonc` 자체 측정값을 Dittus-Boelter/Gnielinski와 비교(SPEC-LIT §35 thermostat 적용 후) | −4.5% / −11.5% (±10% / ±20–25% 밴드 모두 안쪽) |
| 해상 leg 메쉬 해상도(재현) — `cases/channelPeriodicFluxLowRe.jsonc`의 최악 벽인접 y+와 y+<20 셀 수 | y+ = 0.00175, 400개 중 192개 (두 조건 모두 충족) |
| 해상 leg Nusselt 검증치(재현, SPEC-LIT §35) — 같은 케이스, Dittus-Boelter/Gnielinski와 비교 | +8.1% / +16.3% (DB 밴드 안쪽, Gnielinski 밴드 밖 — 게이트 부분 닫힘) |
...
**검증 완료 — SPEC-LIT §34에 따라 진짜 2차원 평면 채널로 재구성.** 두
메쉬에 동일한 벽 열유속 `q_w`를 고정하고 — 각자 자기 자신의 ΔT를
예측하게 하고 — 그 결과를 Dittus & Boelter (1930)와 Gnielinski (1976) 대비
Nusselt 수로 비교하면 벽함수 쪽 게이트가 닫힙니다. 이 게이트가 3차원
덕트에서 돌던 것은 JSONC가 `empty`를 아직 표현하지 못했기 때문일 뿐이었고
(§34.1), 이제 `cases/channelPeriodicFluxWF.jsonc`는 스트림방향 cyclic,
앞뒤 `empty`, 위아래 가열벽뿐입니다 — 이전 수치를 그대로 옮기지 않고
다시 돌렸고, bit 단위로 동일한 고정점에 수렴합니다(`standard`
wallTreatment, y+ ≈ 57.7): `q_w` = 500 W/m², `T_w` = 316.86 K(열 벽함수가
진단), `T_b` = 292.92 K, `U_b` = 5.3696 m/s. 진짜 평면 채널에서는
가열-둘레와 젖은-둘레 관례가 일치합니다(두 벽 모두 가열되니 셋째·넷째
벽을 놓고 다툴 일이 없음) — 그래서 `D_h` = 2H = 0.08 m 하나뿐이고, Re =
28,638, 측정된 Nu = 65.24는 **Gnielinski의 −4.5%**(±10%)와
**Dittus-Boelter의 −11.5%**(±20–25%) — 두 밴드 모두 안쪽입니다. 힘-균형
교차검증은 `U_b/u_tau` = 19.23을 주는데, 완전발달 평면 채널이 주는
15–17에 대해 — 이번에는 옆벽 저항 관련 단서를 붙일 필요가 없습니다,
더 이상 셋째·넷째 벽이 없으니까요. `ofgpu-validate`의
`check_thermal_wall_function_gate_verdict_replay`가 이 재구성된 측정치를
매 실행마다 영구적으로 재현합니다.

**속도장은 검증을 통과했고, 해상 leg의 에너지 방정식이 표류하던 문제는
SPEC-LIT §35가 고쳤습니다.** `LaunderSharmaKE`(SPEC-LIT §33)는 지금 가능한
모든 면에서 검증을 통과합니다 — 감쇠함수의 해석적 극한은
정확하고(`ofgpu-validate`), 벽이 해상된 페리오딕 채널에서 점성 서브레이어
`u+ = y+`를 1% 이내로, 로그 법칙을 1% 이내로 재현하며, SPEC-LIT §34의
재구성이 목표했던 바로 그 지점 — 자기 자신의 해상 leg, 같은 방식으로
재구성한 `cases/channelPeriodicFluxLowRe.jsonc`(`empty` 앞뒤, 옆벽 자체가
사라짐) — 도 속도장을 반올림 수준까지 수렴시킵니다(`|U|` 잔차
2×10⁻¹²), `U_b/u_tau` = 17.35로 벽함수 leg 자신의 19.23보다도 15–17
목표에 더 가깝습니다. **이전 라운드가 검증하지 못한 채 남겨둔 덕트-모서리
가설이 확인되었습니다** — 모서리를 없애자 속도 붕괴가 실제로 사라졌습니다.
검증을 통과하지 못했던 것은 에너지 방정식입니다: `T_b`/`T_w`가 실행을
아무리 늘려도 감쇠하지 않는 듯한 작고 선형적인 속도로 계속 상승했습니다.
SPEC-LIT §35가 이유를 진단했습니다: 모든 열 경계가 Neumann인 닫힌
스트림방향-주기 도메인은 정상상태 온도 방정식이 순수 Neumann이 되어 더하는
상수만큼 부정확해지고, 그 수준은 단순히 처음 준 값을 그대로 유지합니다
(초기 온도 293.15 K와 400 K를 각각 준 두 실행이 서로 다른 체적평균
상태로 수렴). §35.1의 해법은 도메인 자체의 체적평균 `T`에 거는 비례
제어기로, 기존의 고정 `-heaterPower` 싱크를 대체합니다 — 두 채널 케이스
모두 이제 `sources[]`에 이를 갖고 있습니다. 이 진단이 요구한 회귀 검증은
이제 확실히 통과합니다: 해상 leg를 T0 = 293.15 K와 T0 = 400 K에서 각각
다시 돌리면 `T_mean`·`T_b`·`U_b`·thermostat 출력 모두 마지막 출력
자릿수까지 동일하게 수렴하고, 표류 자체가 사라졌습니다(단순히 느려진 게
아니라 — `T`가 5,000반복부터 39,999반복까지 bit 단위로 고정). 열평형은
벽함수 leg에서는 반올림 수준으로 닫히고(thermostat 출력 대 측정 벽열,
차이 2.8×10⁻⁷ W), 해상 leg에서는 2.8%로 닫힙니다 — 이 그레이딩된 메쉬
고유의 압력 솔버 허용오차 바닥으로 추적되며(더 조이면 발산), 조정하지
않고 그대로 보고합니다. **게이트 자체: 벽함수 leg는 여전히 두 상관식
모두를 통과하고(Nu = 65.24, 변화 없음), 해상 leg는 이제 실제 정상상태
Nu = 73.40을 갖습니다 — Dittus-Boelter의 ±20-25% 밴드는 안쪽(+8.1%)이지만
Gnielinski의 더 좁은 ±10% 밴드는 바깥(+16.3%), 6포인트 차이입니다.** 이는
이전의 폭주하던 표류와는 질적으로 다르고 훨씬 작은 문제이며,
Launder-Sharma 자체의 근벽 열전달 예측을(§33.3이 독립적으로 검증한 것은
운동량 로그법칙뿐입니다) 가장 유력하게 시사할 뿐, 또 다른 에너지 회계
결함은 아닙니다. 전체 수치(앞선 덕트 시대의 대체된 시도들, 벽법칙 표,
SPEC-LIT §35의 전체 진단과 해법 포함)는 `docs/07-fire-solver.md` §1.1을
보십시오.
