...
| 열 벽함수 게이트, Nusselt 검증치(재현) — `cases/channelPeriodicFluxWF.jsonc` 자체 측정값을 Dittus-Boelter/Gnielinski와 비교 | −4.5% / −11.5% (±10% / ±20–25% 밴드 모두 안쪽) |
...
**여전히 열려 있음 — 이번에는 완전히 새로운, 세 번째 이유로.**
`LaunderSharmaKE`(SPEC-LIT §33)는 지금 가능한 모든 면에서 검증을
통과합니다 — 감쇠함수의 해석적 극한은 정확하고(`ofgpu-validate`), 벽이
해상된 페리오딕 채널에서 점성 서브레이어 `u+ = y+`를 1% 이내로, 로그
법칙을 1% 이내로 재현하며, SPEC-LIT §34의 재구성이 목표했던 바로 그
지점 — 자기 자신의 해상 leg, 같은 방식으로 재구성한
`cases/channelPeriodicFluxLowRe.jsonc`(`empty` 앞뒤, 옆벽 자체가
...
고쳐지지 않은 것은 에너지 방정식입니다: `T_b`/`T_w`가 실행을 아무리
늘려도 감쇠하지 않는 듯한 작고 선형적인 속도로 계속 상승합니다(150,000
반복까지 확인 — 첫 측정 지점의 10배 — 그 동안 `U_b`와 메쉬 자체의
y+/y+<20 셀 수는 bit 단위로 고정됨) — Nu는 15,000 반복에서는
Dittus-Boelter 밴드 안쪽에서 출발하지만 150,000 반복에서는
**Dittus-Boelter의 +31%, Gnielinski의 +41% — 두 밴드 모두 바깥**으로
표류합니다. 네 가지 원인을 직접 배제했습니다(동일 메쉬에서 `kEpsilon`으로
바꿔도 같은 표류 재현, 훨씬 완만한 격자 조정도 표류, 에너지 방정식
선형해법 허용오차를 엄격하게 조여도 변화 없음, `-heaterPower`가 닫힌 형태
`-2 q_w A_wall`이라 부과된 열유속과 상쇄 싱크가 정확히 같으므로 `-sealed`도
변화 없음). 가장 유력한 설명은 `q_w`와 체적 싱크가 둘 다 온도와 무관하고,
부력이 꺼져 있으며, 이 도메인 어디에도 `T`의 디리클레 조건이 없어 도메인
평균 온도를 되돌릴 항이 이산 에너지 균형에 전혀 없다는 것입니다 — 그런데
왜 조(粗)격자 벽함수 메쉬는 정확한 고정점에 도달하고 미세한 해상 메쉬는
그러지 못하는지는 이번 작업으로 밝히지 못한, 이전에는 보이지 않았던 새로운
한계로, 맞춰서 없애는 대신 그대로 보고합니다. 전체 수치(앞선 덕트 시대의
