...
| 검증 | 단위 시험 1,608개(모든 타깃, lib 1,460개), `ofgpu-validate` 747개 항목 |
...
영역이 존재하지 않기 때문입니다. 위 다섯 케이스에서 16조각일 때의 비용은
전체 메시 반복 횟수 대비 **1.03배–1.84배**이며, 가장 큰 메시에서 가장
...
발표하는 것은 측정값입니다. 반복 1회의 셀당 비용(80만 셀에서 0.90 ns), 반복당
...
| `ofgpu-decompose` | 케이스를 분할하여 한 프로세스에서 실행하고 비트 변화 여부를 보고 (SPEC-LIT §71). 그 분할의 모든 재라벨링에 대해 축약을 수행하고 값이 움직였는지 보고 (§72). 이어서 분산 PCG/PBiCGStab으로 실제 해를 구하여 장과 반복 횟수가 움직였는지 보고하고, 블록 국소 DIC/DILU의 반복 횟수 사다리와 반복당 비용을 함께 출력 (§73) |
