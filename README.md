...
| 검증 | 단위 시험 1,595개(모든 타깃, lib 1,447개), `ofgpu-validate` 747개 항목 |
...
**여전히 한 프로세스, 한 GPU입니다.** MPI도 NCCL도 두 번째 장치도 없으며,
조각들은 한 장의 카드 위에서 장치–장치 복사로 데이터를 주고받습니다. 게이트가
실행하는 것은 전체 메시에서 조립한 행렬에 대한 고정 반복 Jacobi 스윕이며,
이것이 현재 게이트로 세울 수 있는 최대치입니다. 조각 간 축약(reduction)은
분할 불변이 아니고 §70.5의 조립 커널 16개도 마찬가지이기 때문입니다. 둘 다
§71.7에서 이름을 들어 거부하며, 다색 전처리기, FFT 압력 백엔드, 분할 입출력,
Lagrangian 입자도 함께 거부합니다. **METIS 5.2.x는 Apache-2.0**임을 `LICENSE`
원문으로 확인하였으나 의도적으로 연결하지 않았습니다. §71.2가 세 가지 이유를
제시하며, 결정적인 이유는 METIS의 결과가 빌드에 따라 달라져 분할이 메시가
아니라 링크된 라이브러리의 성질이 되어 버린다는 점입니다.
...
| `ofgpu-decompose` | 케이스를 분할하여 한 프로세스에서 실행하고 비트 변화 여부를 보고 (SPEC-LIT §71) |
