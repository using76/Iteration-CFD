...
| A driver may not default a MOMENTUM equation's convection entry to the `bounded` form. §3.1's correction subtracts `V_P (div u)_P` on the argument that `div u` is a convergence error; §25.1 makes it a prescribed constraint, so on a momentum equation it removes real converged momentum and the §32.5.2 force balance fails by that amount - measured at -3.787% and closed to -0.000% by honouring the case's own entry. Where the correction IS physics (§26's energy equation) it is applied unconditionally and the flag is documented as not being a setting | `src/momentum.rs`, `src/energy.rs`, `src/bin/fire.rs` | §3.1, §13.4.1, §32.5.5 |
