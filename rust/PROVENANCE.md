...
| File | SPEC-LIT | Primary sources |
|---|---|---|
| `src/energy.rs`, `cuda/energy.cu` | §25, §26 | Rehm & Baum, *J. Res. Natl. Bur. Stand.* 83 (1978) 297-308 (the `p = p0(t) + p~` split, the divergence constraint and `p0` evolution); Majda & Sethian, *Combust. Sci. Technol.* 42 (1985) 185 (low-Mach acoustic filtering); the FDS Technical Reference Guide (McGrattan et al., NIST SP 1018, public domain — `reference/fds` read and adapted for the SHAPE of the divergence constraint and the sealed/open `p0` bookkeeping, acknowledged per SPEC-LIT §0; no FDS code copied); Patankar (1980) §4.2 for the `Su`/`Sp` linearisation the `EnergySources` registry hands to `fvm_su`/`fvm_sp`. **Kays, *ASME J. Heat Transfer* 116 (1994) 284-295** is CITED in SPEC-LIT §32.5.5 and `docs/07-fire-solver.md` §1.1 and implemented by NOTHING: it is the reference behind the named, unmeasured hypothesis that this file's case-wide constant `Pr_t` is what leaves §32's resolved leg 14% above Gnielinski. Listed here so a reader can tell a citation that drives code from one that only names an open question |
...
| A retired case that a LATER §13.4 rule has made unrunnable is MARKED, not resurrected: `channelThermalLowRe`/`channelPeriodicLowRe` name `lowRe` with `kEpsilon`, which §33's rule (added after their runs) refuses. Editing them to `LaunderSharmaKE` would make the file no longer the file its recorded numbers came from, so the record is kept and the published commands are commented out with the reason | `cases/channelThermalLowRe.jsonc`, `cases/channelPeriodicLowRe.jsonc`, `cases/README.md` | §13.4.3, §33 |
| Every source file's provenance declaration is checked in BOTH directions — the file names its category and carries "No GPL-licensed source was consulted", and `PROVENANCE.md` names the file — so an acquirer auditing file by file and one auditing the table reach the same answer. 105/105 either way | all of `rust/src`, `rust/cuda`, `rust/tests`, `rust/build.rs`; this file | §0, `LICENSING.md` §6 |
