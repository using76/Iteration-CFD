...
**1차 사용자는 화재·연기·환경 시뮬레이션입니다** (이 프로젝트의 출발점이자
...
**Q2. 컷셀에서 경계층은?** 문제 부류로 갈라집니다. 화재·플룸·건물 규모
유동은 벽이 거칠고 y+가 수백~수천이라 직교 셀 + 벽함수가 표준 관행입니다
(FDS가 정확히 이 구조로 20년 검증). 반면 공력 해석은 y+ 제어가 필요한데
컷셀에서는 첫 셀 높이가 체적 격자 해상도에 종속되어 **제어가 불가능**합니다.
→ **계획 수정: 컷셀 경로의 대상을 화재·AEC·플룸으로 명시**하고, 공력류는
...
**Q6. 누가 사용자인가?** 이 계획이 가장 답해야 했던 질문. 이 프로젝트는 FDS
비교 리포트에서 출발했고, 대표 케이스는 900 °C 플룸이며, 회사는
시뮬레이션 기반 디지털 트윈을 지향합니다. 즉 **화재·연기·환경 시뮬레이션이
본업**이고, "표면 입력 → 직교 격자 → VDB/Omniverse 출력"은 정확히 그
...
4. 컷셀 생성기(10단계) 별도 설계 문서 — 대상은 화재·AEC 유동으로 한정
