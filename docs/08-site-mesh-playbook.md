...
`sink_m 1.5`는 건물 바닥을 지면(5.0) 아래 3.5 m로 내리지만 해수면은 3.05라, 해안에 걸친 프리즘
밑에는 0.45 m 두께의 유체층이 남는다. 세 원천 격자에서 두께 게이트가 못 고친 셀(τ 0.024, 북동
모서리 (631, 631, z 3.3))이 바로 그 층이었고, 수정 라운드를 8로 올려도(τ 0.036, 밀기 거부 1) 남았다.
`solids.base_below_sea_m 1.0`은 모든 바닥을 해수면 아래 1 m(2.05)까지 내린다 — 육지에선 땅속,
물 위에선 트림이 잘라내므로 층이 사라진다. README의 "바닥이 트림면에 닿으면 5 cm 띠" 주의는
1 m 여유로 피한다.

### 2d. 도메인 범위: 트림은 수렴 시험용, 납품은 전체

±650 m 트림(`trim.shrink_xy_m 600`)은 자체 스테디 솔버가 먼 경계에서 발산하던 문제를 피하려고 넣은 것이다.
Fluent 납품 격자를 그 레시피로 만들자 북서쪽 산(지형 z 최대 85 m, x −1,250~−1,157·y 1,046~1,086)이 통째로
잘려 사용자가 "형상이 너무 다르다"고 거부했다. 세 원천 최종본은 전체 ±1,250 m로 다시 만들었다: `shrink_xy_m 0`,
`max 20`(성장 필드가 700 m에서 상한에 닿아 외곽 링이 20 m급) → 1,610만 셀로 ±650 판(1,644만)과 같고, 원천 200 m
안엔 SICN 0.3 미만 셀이 없다. 대가는 먼 지형의 꼬리 — 쐐기 27곳(±650 판 7곳)·미해결 틈 598곳(127곳)에서
min SICN 0.045, 0.2 미만 203개(186개가 650 m 밖). 도메인 범위는 격자 작업 전에 사용자에게 먼저 확인한다.
