...
3. **틈 규칙** `sizes.gap_ratio 2.5`: 표면 격자 삼각형마다 안쪽 법선 방향으로 마주보는(법선 내적 < −0.5)
   ...
   잔재) 444곳이 나왔고, 재표본을 3 m/3000면으로 굵게 하는 것이 답이다.
...
5. **변환**: `ofgpu-convert-mesh … -fluent … -fluentType pool_<name>=velocity-inlet`. 밀봉 포켓은 자동
...
- 셀 하나가 면 기반 부피 −30 m³로 나오는 변환 문제(삼각형 대조 후에도 재현) — `cell_faces.py`로 추적 중.
- 선박 재표본 3 m/3000면으로 상부구조 틈 제거, 세 케이스를 최종 레시피로 재생성, 6회 시험, 50회 시험.
- Fluent 음수 부피 경고의 원인(방향 규약 vs 납작 셀) 확정 → 변환기 기본 규약 결정.
