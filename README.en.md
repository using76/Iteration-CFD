...
| | |
|---|---|
| JSONC case | One JSON file (comments and trailing commas allowed) naming mesh, physics, boundaries, numerics and the fire block — the schema is generated by `schemars` from the same types the reader uses, so the two cannot disagree |
